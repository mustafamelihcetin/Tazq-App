...
if map_start_idx != -1 and remaining_empty_state_idx != -1:
    ...
    # Fix Set<number> to Set<string> for selectedIds
    for i, line in enumerate(lines):
        if "useState<Set<number>>(new Set())" in line:
            lines[i] = line.replace("useState<Set<number>>(new Set())", "useState<Set<string>>(new Set())")
