...
const memoComponent = `
const MemoizedTaskItem = React.memo(({ task, i, theme, isDark, highlightedId, isBulkMode, isSelected, language, t, showSwipePeek, priorityColor, handleDelete, handleToggleExpand, handleLongPress, handleBulkSelect, handleToggle, toggleSubtask, completingIds, expandedTasks }) => {
                                ...
                                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                                    <View style={{ flexShrink: 1, opacity: isTaskCompletedLocally ? 0.4 : 1, transform: [{ scale: isTaskCompletedLocally ? 0.97 : 1 }] }}>
                                        <Text style={[
                                            ...
                                            isTaskCompletedLocally && { textDecorationLine: 'line-through' }
...
const mapStr = 'visibleTasks.map((task, i) => {';
let mapStartIndex = content.indexOf(mapStr);
let remainingCountStr = '{remainingCount > 0 && (';
let remainingStartIndex = content.indexOf(remainingCountStr, mapStartIndex);
let remainingEndIndex = content.indexOf(')}', remainingStartIndex + 100);

if (mapStartIndex !== -1 && remainingEndIndex !== -1) {
    const startStr = content.substring(0, mapStartIndex);
    const endStr = content.substring(remainingEndIndex + 2);
