...
2.5 Sağlık Verileri (Apple Sağlık / Health Connect) — Yalnızca Cihazda İşlenir
Bu özellik varsayılan olarak KAPALIDIR ve yalnızca sizin açık izninizle, uygulama ayarlarından etkinleştirilir:
   – Apple Sağlık (iOS) veya Health Connect (Android) üzerinden YALNIZCA şu veriler okunur:
      • Uyku süreniz — "uyku" alışkanlığınızı otomatik işaretlemek için.
      • Adım sayınız, yürüme/koşma mesafeniz ve antrenman seanslarınız (tür ve süre) — dönemsel modlarınızdaki yürüyüş, koşu ve antrenman görevlerini otomatik tamamlamak ve spor modunda günlük adım sayınızı göstermek için.
   – Bu veriler YALNIZCA SALT OKUNUR erişimle alınır; Apple Sağlık veya Health Connect üzerine hiçbir kayıt YAZILMAZ.
   – Her izin ayrı ayrı verilir: yalnızca uykuyu paylaşıp hareket verilerini paylaşmamayı (veya tersini) seçebilirsiniz. İzin verilmeyen veri okunmaz ve ilgili özellik sessizce devre dışı kalır.
   – Bu sağlık verisi YALNIZCA CİHAZINIZDA işlenir; sunucularımıza gönderilmez, tarafımızca saklanmaz, üçüncü kişilerle paylaşılmaz ve hiçbir şekilde pazarlama veya reklam amacıyla kullanılmaz.
   – İzni istediğiniz zaman uygulama ayarlarından veya cihazınızın Sağlık/Health Connect ayarlarından geri çekebilirsiniz. Özellik kapatıldığında hiçbir sağlık verisi okunmaz.
...
2.5 Health Data (Apple Health / Health Connect) — Processed On-Device Only
This feature is OFF by default and is enabled only with your explicit permission from the app settings:
   – ONLY the following data is read from Apple Health (iOS) or Health Connect (Android):
      • Your sleep duration — to auto-mark your "sleep" habit.
      • Your step count, walking/running distance and workout sessions (type and duration) — to auto-complete the walking, running and training tasks in your seasonal modes, and to show your daily step count in sport mode.
   – This data is accessed READ-ONLY; nothing is ever written back to Apple Health or Health Connect.
   – Each permission is granted separately: you may share sleep but not activity (or the other way round). Data you do not grant is never read and the related feature simply stays off.
   – This health data is processed ON YOUR DEVICE ONLY; it is not transmitted to our servers, not stored by us, not shared with any third party, and never used for marketing or advertising.
   – You can revoke access at any time from the app settings or your device's Health/Health Connect settings. When the feature is disabled, no health data is read.
